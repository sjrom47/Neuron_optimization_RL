...
def default_model_path_candidates(model_type, waveform_type, criterion_type, cell_id=36):
    stem = f"weights/{model_type}_{waveform_type}_{criterion_type}_cell{cell_id}_opt"
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="plots/compare_vs_bayesopt_baseline",
...
def run_rl_episode(env, model, is_recurrent):
...
def evaluate_rl(args, seeds, run_dir):
    ...
    for ep_idx, seed in enumerate(seeds):
        ...
        ep_best_reward, ep_best_info = run_rl_episode(env, model, is_recurrent)
...
def print_comparison_summary(args, run_dir, rl_results, bo_results, episode_plot_paths):
...
def main():
    ...
    print_comparison_summary(args, run_dir, rl_results, bo_results, episode_plot_paths)
