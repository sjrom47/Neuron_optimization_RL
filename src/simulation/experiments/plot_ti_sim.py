...
SAVE_PATH_ti = os.path.join(str(TI_RESULTS_DIR), 'TI/TI_target_region')
if not os.path.exists(SAVE_PATH_ti):
    os.makedirs(SAVE_PATH_ti)

SAVE_PATH_no_ti = os.path.join(str(TI_RESULTS_DIR), 'TI/TI_non-target_region')
...
for l in range(len(amp_level)):
    ...
    if int(amp_level[l]) == 1100:
        ...
        plot_response(coord=points_samples_ti, values1=ti_region_ti_pv, values2=ti_region_ti_pyr, y_displace=2, title=None, savepath=os.path.join(str(TI_RESULTS_DIR), 'Figs5Main/Fig5-j'), show=True, target=True)
        plot_response(coord=points_samples_no_ti, values1=no_ti_region_ti_pv, values2=no_ti_region_ti_pyr, y_displace=6, title=None, savepath=os.path.join(str(TI_RESULTS_DIR), 'Figs5Main/Fig5-k'), show=True, target=False)
