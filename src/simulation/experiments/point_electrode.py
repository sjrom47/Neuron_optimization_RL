...
from simulation.fields.elec_field import ICMS
from simulation.experiments.helper import fibonacci_sphere, plot_electrode_and_neuron
from simulation.neuron.model import NeuronSim
from simulation.experiments.pulse_train import PulseTrain_square
from simulation.paths import CELLS_DIR, PROJECT_ROOT, SAVE_STATE_DIR, TI_RESULTS_DIR


def pointElec_simulation(
    num_electrode,
    amplitude,
    pulse_width,
    period,
    ...
    elec_location_ICMS = fibonacci_sphere(
        num_electrode
    )  ## Sampling approximately uniformly spaced electrode locations from a unit sphere
    elec_location_ICMS = (
        elec_location_ICMS * dist
    )  ## Scaling the radius of sphere to the dist variable

    angle_pv = np.array(
        [0, 0]
    )  ## parameter used for specifying rotation of PV morphology
    angle_pyr = np.array(
        [0, 0]
    )  ## parameter used for specifying rotation of Pyr morphology

    loc_pyr = np.array(
        [0, 0, 0]
    )  ## parameter used for specifying location of Pyr morphology
    loc_pv = np.array(
        [0, 0, 0]
    )  ## parameter used for specifying location of PV morphology
    ...
    if plot_neuron_with_electrodes:
        ...
        if cell_type in [6, 7, 35, 36]:
            neuron = NeuronSim(
                human_or_mice=human_or_mice, cell_id=cell_type, temp=temp, dt=dt
            )
            coord = neuron._translate_rotate_neuron(pos_neuron=loc_pyr, angle=angle_pyr)

            ## Plot ICMS Electrode With Neuron
            plot_electrode_and_neuron(
                coord_elec=elec_location_ICMS * 10**3,
                coord=coord,
                savepath=savepath_curr,
            )
    ...
    amp_array, time_array = pulse_train_square.amp_train(
        amp=amplitude,
    ...
    for l in range(len(elec_location_ICMS)):
        ...
        print(
            "Electrode Location Chosen for Simulation: x=%s mm, y=%s mm, z=%s mm"
            % (
                str(round(elec_location_ICMS[l, 0], 3)),
                str(round(elec_location_ICMS[l, 1], 3)),
                str(round(elec_location_ICMS[l, 2], 3)),
            )
        )
        elec_field = ICMS(
            x=elec_location_ICMS[l, 0],
        ...
        print(
            "Electric Field Simulator Loaded! Time Taken %s s"
        ...
        if cell_type in [6, 7, 35, 36]:
            ...
            neuron = NeuronSim(
                ...
                elec_field2=elec_field2,
            )  ## Initializing neuron model
            print("Cell Id chosen %d." % (int((cell_type))))
            neuron._set_xtra_param(
                angle=angle_pyr, pos_neuron=loc_pyr
            )  ## Setting Extracellular Stim Paramaters
            delay_init, delay_final = (
                2000,
                5,
            )  ## ms, delay added to the stimulation before and after applying stimulation
            ...
            if not os.path.exists(save_state):
                ...
                neuron.stimulate(
                    ...
                    amp_array=amp_array,
                    amp_array2=amp_array2,
                    sampling_rate=sampling_rate,
                    delay_init=delay_init,
                    delay_final=delay_final,
                    save_state_show=save_state_show,
                )
                print(
                    "Save State Generated! Time Taken %s s"
                    % (str(round(time.time() - start_time, 3)))
                )

            start_time = time.time()
            print("Simulation for Neuron Started...")
            results = neuron.stimulate(
                time_array=time_array,
                amp_array=amp_array,
                scale1=1,
                sampling_rate=sampling_rate,
            ...
            print(
                "Simulation Finished! Time Taken %s s"
                % (str(round(time.time() - start_time, 3)))
            )

            soma_recording_filtered, t_filtered = neuron.save_soma_recording(
                delay_init=delay_init
            )
...
def main():
    ...
    parser.add_argument(
        "--num_electrode", type=int, default=1, help="Number of electrodes"
    )
    parser.add_argument(
        "--amplitude", type=float, default=10000, help="Amplitude of stimulation"
    )
    parser.add_argument(
        "--pulse_width", type=float, default=100, help="Pulse width in ms"
    )
    parser.add_argument("--period", type=float, default=200, help="Pulse period in ms")
    parser.add_argument(
        "--total_time", type=float, default=1000, help="Total stim duration in ms"
    )
    parser.add_argument(
        "--cell_type", type=int, default=6, help="Cell type to stimulate"
    )
    parser.add_argument(
        "--plot_neuron_with_electrodes",
        type=bool,
        default=False,
        help="Show neuron with electrodes",
    )
    args = parser.parse_args()
    soma, t = pointElec_simulation(
    ...
    plt.plot(t, soma[0], label="Soma Voltage")
    plt.xlabel("Time (ms)")
    plt.ylabel("Voltage (mV)")
    plt.title("Soma Voltage Over Time")
    plt.legend()
    plt.show()
