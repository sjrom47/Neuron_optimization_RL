...
from simulation.fields.elec_field import UniformField
...
###################################################################################
###################################################################################
SAVE_PATH = os.path.join(str(TI_RESULTS_DIR), 'PointElectrodeSim/Results_uniform')
...
for l in range(len(unit_vec)):
    ...
    if not LOAD_DATA_FLAG:
        ...
        save_state = os.path.join(str(SAVE_STATE_DIR),'human_or_mice'+str(ray.get(human_or_mice))+'cell-'+str(ray.get(cell_id_pyr))+'_Temp-'+str(ray.get(temp))+'C_dt-'+str(ray.get(dt)*10**3)+'us_delay-'+str(ray.get(delay_init))+'ms.bin')
        ...
        ## Checking if Save State for Pyramidal neuron model Exists and if not then creating one
        save_state = os.path.join(str(SAVE_STATE_DIR),'human_or_mice'+str(ray.get(human_or_mice))+'cell-'+str(ray.get(cell_id_pv))+'_Temp-'+str(ray.get(temp))+'C_dt-'+str(ray.get(dt)*10**3)+'us_delay-'+str(ray.get(delay_init))+'ms.bin')
        if not os.path.exists(save_state):
            start_time = time.time()
