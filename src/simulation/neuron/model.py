...
from simulation.paths import HOC_DIR, NEURON_ASSETS_DIR, SAVE_STATE_DIR, ensure_dir
...
class NeuronSim:
    ...
    def __init__(self, human_or_mice=0, cell_id=None, temp=37, dt=0.025, show_morphology_old=False, elec_field=None, elec_field2=None):
        ...
        h.load_file("nrngui.hoc")
        h.chdir(str(NEURON_ASSETS_DIR))
        h.load_file(str(HOC_DIR / "interpCoordinates.hoc"))
        h.load_file(str(HOC_DIR / "setPointers.hoc"))
        h.load_file(str(HOC_DIR / "cellChooser.hoc"))
        h.load_file(str(HOC_DIR / "setParams.hoc"))
        h.load_file(str(HOC_DIR / "editMorphology.hoc"))
        if show_morphology_old:
            h.load_file(str(HOC_DIR / "calcVe.hoc"))
            h.load_file(str(HOC_DIR / 'color_max.hoc'))
            h('color_plotmax()')
...
"L5_TTPC1_cADpyr232_5"]
            ...
            def stimulate(self, time_array, amp_array, amp_array2=None, sampling_rate=1e5, delay_init=2, delay_final=2, plot=False, Vinit=-65, time_stamps=None, save_state_show=False):
                save_state = ensure_dir(SAVE_STATE_DIR)
