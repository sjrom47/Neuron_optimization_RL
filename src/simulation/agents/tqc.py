...
class TQCClass:
    ...
    def train(self):
        ...
        run_dir = str(ensure_dir(PLOTS_DIR / f"{timestamp}_tqc_cell{self.cell_id}"))
        self.model.learn(
        ...
        self.model.save(str(ensure_dir(WEIGHTS_DIR) / f"tqc_{self.waveform}_{self.criterion}_cell{self.cell_id}_opt"))
